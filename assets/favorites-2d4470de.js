import{o as k,s as p,h as F,n as h,a as A,f as B,c as $}from"./order-3badd72e.js";let l=new Set;const s={cardsFavorites:document.querySelector(".list_cards_favorites"),categoriesFavorites:document.querySelector(".favorites_categories"),allCategories:null};s.cardsFavorites.addEventListener("click",_);function S(t){if(t.length===0){s.categoriesFavorites.innerHTML="";return}const e=t.map(o=>`<li><button class="main-button recipe-item-see category-btn" type="button">${o}</button></li>`),n=l.size===0?"green-button":null;e.unshift(`<li><button id="allcat" class="main-button recipe-item-see category-btn ${n}" type="button">All categories</button></li>`),s.categoriesFavorites.innerHTML=e.join(""),s.categoriesFavorites.addEventListener("click",P),s.allCategories=document.querySelector("#allcat")}function w(t,e,n){const o=(e-1)*n,d=e*n,v=T(t).slice(o,d);v.length?s.cardsFavorites.innerHTML=v.join(""):s.cardsFavorites.innerHTML=`<div class="not_favorites">
					...
					</div>`,F()}function P({target:t}){if(t.classList.contains("main-button")){if(N(1),t===s.allCategories){const e=s.cardsFavorites.children,n=s.categoriesFavorites.children;for(let o=1;o<n.length;o++)n[o].children[0].classList.remove("green-button");for(let o=0;o<e.length;o++);t.classList.add("green-button"),l.clear(),c(1);return}l.has(t.textContent)?(l.delete(t.textContent),t.classList.remove("green-button")):(l.add(t.textContent),t.classList.add("green-button"),s.allCategories.classList.remove("green-button")),l.size||s.allCategories.classList.add("green-button"),c(1),j()}}function j(){const t=s.categoriesFavorites.querySelectorAll(".green-button");if(s.categoriesFavorites.children.length-1===t.length){s.allCategories.classList.add("green-button"),l.clear();for(const n of t)n.classList.remove("green-button")}}function c(t){const e=l.size?m.filter(({category:n})=>l.has(n)):m;w(e,t,f),W(e.length)}function _({target:t}){t.classList.contains("recipe-item-see")&&k(t.dataset.id)}window.addEventListener("resize",x);function x(){window.innerWidth<768&&location.reload()}let r=0,a=0,m=[];const f=window.innerWidth>767?12:9,q=document.querySelector(".list_cards_favorites"),g=document.querySelector(".pagination-buttons");q.addEventListener("click",I);y();function E(){let t=[];try{t=JSON.parse(localStorage.getItem("favorites"))}catch{h.Notify.info("There are no recipes added to favorites.")}return t}async function M(){const t=E();let e=[];if(t.length){const n=t.map(async o=>await B(o));e=await Promise.all(n)}return e}function y(){h.Loading.dots(),M().then(t=>{m=t,a=1;const e=H(t);S(Array.from(e)),C(t.length),w(m,1,f)}).catch(()=>h.Notify.info("There are no recipes added to favorites.")).finally(h.Loading.remove(1e3))}function T(t){let e=[];return t&&t.forEach(n=>{e.push($(n,"in-favorites",`${p}#icon-like-full`))}),e}function H(t){const e=new Set;return t&&t.forEach(n=>{e.add(n.category)}),e}function I(t){t.target.classList.contains("js-like")&&(A(t),y())}function C(t){if(t<f){g.innerHTML="";return}r=Math.ceil(t/f);const e=f===9?3:4,n=r>=e?e:r;let o="";for(let i=1;i<=n;i+=1)i!==e?o+=`<button class="pagination-btn btn-js btn-pg" data-id="${i}" data-value="${i}">${i}</button>`:o+=`<button class="pagination-btn btn-js btn-pg" data-id="${i}" data-value="${i}">...</button>`;const d=`${p}#icon-small-right`,b=`${p}#icon-small-left`;let v=` <div class="back-buttons additional">
